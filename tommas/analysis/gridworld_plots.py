...
from tommas.analysis.representation_metrics_gridworld import load_representation_data, get_representation_similarity_score

plot_dirpath = "data/imgs/gridworld/"
...
def save_performance_plot(df, n_past=5):
    ...
    sns.boxplot(x=df5["char_embedding_size"], y=df5[("action_loss", "mean")] + df5[("goal_loss", "mean")],
                hue=df5["Architecture"], ax=axes[0, 0])
    axes[0, 0].set(xlabel="embedding size", ylabel="overall loss")
    sns.boxplot(x=df5["char_embedding_size"], y=(df5[("action_acc", "mean")] + df5[("goal_acc", "mean")]) / 2,
                hue=df5["Architecture"], ax=axes[0, 1])
    axes[0, 0].set(xlabel="embedding size", ylabel="overall loss")
    axes[0, 1].set(xlabel="embedding size", ylabel="overall accuracy")

    sns_char_boxplot(df5, "action_loss", axes[1, 0])
    axes[1, 0].set(xlabel="embedding size", ylabel="action loss")
    sns_char_boxplot(df5, "action_acc", axes[1, 1])
    axes[1, 1].set(xlabel="embedding size", ylabel="action accuracy")

    sns_char_boxplot(df5, "goal_loss", axes[2, 0])
    axes[2, 0].set(xlabel="embedding size", ylabel="goal loss")
    sns_char_boxplot(df5, "goal_acc", axes[2, 1])
    axes[2, 1].set(xlabel="embedding size", ylabel="goal accuracy")

    plt.savefig(plot_dirpath + "gridworld_" + filename)
...
def save_dist(df):
    ...
    plot_embedding_space_as_boxplot(model_name="lstm[64,2]_lstm[64,2]_seed1", ax=axes[0, 0])
    axes[0, 0].set(title="LSTM", ylabel="Embedding Size 64")
    plot_embedding_space_as_boxplot(model_name="ttx[64,8,8]_ttx[64,4,8]_seed1", ax=axes[0, 1])
    axes[0, 1].set(title="Transformer")
    plot_embedding_space_as_boxplot(model_name="lstm[128,2]_lstm[64,2]_seed1", ax=axes[1, 0])
    axes[1, 0].set(ylabel="Embedding Size 128")
    plot_embedding_space_as_boxplot(model_name="ttx[128,8,8]_ttx[64,4,8]_seed1", ax=axes[1, 1])
