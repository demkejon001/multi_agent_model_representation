...
from tommas.env.gridworld_env import RepositionGridworldEnv, GridworldEnv
...
dataset_dirpath = "data/datasets/ia_ja4_reposition_picnic/"
...
def get_random_env(reward_funcs, agents):
    ...
    if agents is None:
        ...
        env = RepositionGridworldEnv(agents, reward_funcs, num_goals, horizon, dim, max_num_walls=max_num_walls,
                                     seed=env_seed)
...
def collect_no_group_data(num_agents, dataset_filename):
    ...
    for _ in range(num_agents):
        ...
        env = get_random_env(reward_funcs, None)
        base_save_dataset([play_episode(env, agents) for _ in range(min_num_episodes_per_agent)], dataset_dirpath, dataset_filename + str(file_idx))
        file_idx += 1
...
def collect_picnic_data(num_agents, dataset_filename):
    ...
    for _ in range(math.ceil(num_agents / len(create_agent_kwargs))):
        for kwargs in create_agent_kwargs.values():
            ...
            env = get_random_env(reward_funcs, picnic_agents)
