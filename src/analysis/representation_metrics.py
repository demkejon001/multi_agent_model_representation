...
from src.viz.embedding_responsibility import cosine_loss, get_responsible_embeddings_silhouette_score, \
    linear_contrastive_loss
from src.analysis.disentanglement import dci
...
representation_metrics = ["cluster_silhouette_score",  "modified_cluster_silhouette_score",
                          ...
                          *agent_specific_param_metrics,
                          ...
                          # 'mixed_trigger_pattern_vs_rest_dim',
...
def create_models_output_df(models_dict, num_agents_per_cluster=100, limited=True, seed=42, n_past=5):
        ...
        for i in range(num_agents_per_cluster):
            if limited:
                ...
                if action_pattern is not None and type(action_pattern[0]) == list:
                    ...
                    cluster_trajs.append(generate_random_traj(agent_type, new_agent_info, seed=seed, num_agents=4,
                                                              opponent_idx=opponent_idx, n_past=n_past))
                    seed += 1
                else:
                    cluster_trajs.append(generate_random_traj(agent_type, agent_info, seed=seed, num_agents=4,
                                                              opponent_idx=opponent_idx, n_past=n_past))
                    seed += 1
            else:
                cluster_trajs.append(generate_random_traj(agent_type, agent_info, seed=seed, num_agents=4,
    ...
    for agent_type, agent_info, opponent_idx in agent_clusters:
...
def append_modified_silhouette_scores_to_df(agent_specific_df):
    ...
    for model_name in models_names:
        all_labels_and_embeddings = agent_specific_df[agent_specific_df["model_name"] == model_name][
            ["agent_type", "starting_action", "n_past", "char_embedding"]]
        for n_past in range(6):
            all_embeddings = []
            labels = []
            group_labels = []
            for (agent_type, starting_action), df in all_labels_and_embeddings.groupby(
                    ["agent_type", "starting_action"]):
                modified_label = agent_type + str(starting_action)
                group_labels.append((agent_type, starting_action))
            ...
            for i, (agent_type, starting_action) in enumerate(group_labels):
                idx = num_embed_per_cluster * i
                label_average = np.average(samples[idx:idx + num_embed_per_cluster])
                data.append({"model_name": model_name,
                             "n_past": n_past,
    ...
    return agent_specific_df.merge(pd.DataFrame(data), how="left",
                                   on=["model_name", "n_past", "agent_type", "starting_action"])
...
def append_dci_scores(agent_specific_df, model_specific_df):
    ...
    for model_name, model_df in tqdm(agent_specific_df[agent_specific_df["n_past"] == n_past].groupby("model_name")):
        ...
        data.append({"model_name": model_name, "n_past": n_past,
                     ...
                     "informativeness_starting_action": informativeness[1],
                     "informativeness_opponent_idx": informativeness[2]})
    df = pd.DataFrame(data)
    return model_specific_df.merge(_df_add_level(df), how="left", on=["model_name", "n_past"])
...
def append_model_specific_param_scores(agent_specific_df, model_specific_df, mask_threshold=.75):
    ...
    return model_specific_df.merge(_df_add_level(pd.DataFrame(agent_type_data)), how="left",
                                   on=["model_name", "n_past"])
...
def append_model_specific_param_lin_scores(agent_specific_df, model_specific_df):
    def get_shared_param_scores():
        param_scores = {"model_name": model_name, "n_past": n_past}
        for agent_param in agent_params:
            embedding_df = agent_df[[agent_param, "char_embedding"]]
            ...
            embeddings = np.concatenate(embeddings, axis=0)
            labels = np.array(labels)
            silhouette = linear_contrastive_loss(embeddings, labels)
            param_scores.update({f"shared_{agent_param}_lin_score": silhouette})
        return [param_scores]
    ...
    def get_agent_type_scores():
        ...
        silhouette = linear_contrastive_loss(embeddings, labels)
        param_scores = {"model_name": model_name, "n_past": n_past,
                        "agent_type_lin_score": silhouette}
        return [param_scores]
...
def append_agent_specific_param_lin_scores(df):
    # Looking at how well we can tell an agent's parameter values from each other as well as from non-existent values, i.e. grim trigger's start action = 0 vs 1 (as well as vs non grim trigger agents)
    def get_agent_param_scores():
        data = []
        for agent_type in agent_types:
            ...
            for agent_param in agent_params:
                ...
                if len(embedding_df[agent_param].unique()) > 1:
                    ...
                    param_embeddings = np.concatenate(embeddings, axis=0)
                    param_labels = np.array(labels)
                    silhouette = linear_contrastive_loss(param_embeddings, param_labels)

                else:
                    silhouette = np.nan
                    feature_dim = np.nan
                param_scores.update({f"{agent_param}_lin_score": silhouette})
            data.append(param_scores)
        return data
...
def _get_model_char_embeddings(model, trajs):
    ...
    for agents_trajs in trajs:
        batch.append(transform([IterativeActionTrajectory([0, 1], traj) for traj in agents_trajs], 0, n_past, np.inf, shuffled_agent_indices=[0, 1, 2, 3]))
    ...
    with torch.no_grad():
        pred = model(input, return_embeddings=True)
        char_embeddings = pred.embeddings[0].cpu().numpy()
    return char_embeddings[:, -1]  # return batch last embedding
...
def lstm_ttx_boxplot_comparison(df, metric, n_past=5, x="char_embedding_size", agg_metric="mean", agent_type=None,
    ...
    if verbose:
        ...
        print("Median (LSTM, TTX): " + lstm_ttx_metric_str(medians))
        print("Average (LSTM, TTX): " + lstm_ttx_metric_str(avgs))
...
def lstm_ttx_agent_param_boxplot_comparison(df, param, n_past=5, x="char_embedding_size", compare=None,
    ...
    if compare is None:
        ...
        lstm_ttx_boxplot_comparison(df, metric=param+"_score", n_past=n_past, x=x, agent_type=agent_type, ax=axes[0])
        lstm_ttx_boxplot_comparison(df, metric=param+"_dim", n_past=n_past, x=x, agent_type=agent_type, ax=axes[1])
        axes[0].set_ylim((-1, 1))
    else:
        fig, axes = plt.subplots(2, 2, figsize=(12, 8))
        lstm_ttx_boxplot_comparison(df, metric=param+"_score", n_past=n_past, x=x, agent_type=agent_type, ax=axes[0, 0])
        lstm_ttx_boxplot_comparison(df, metric=param+"_dim", n_past=n_past, x=x, agent_type=agent_type, ax=axes[0, 1])
        if compare == "shared":
        ...
        lstm_ttx_boxplot_comparison(df, metric=compare_param + "_score", n_past=n_past, x=x, agent_type=agent_type,
                                    ax=axes[1, 0])
        lstm_ttx_boxplot_comparison(df, metric=compare_param + "_dim", n_past=n_past, x=x, agent_type=agent_type,
                                    ax=axes[1, 1])
