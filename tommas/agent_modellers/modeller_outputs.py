...
import torch
from tommas.agent_modellers.modeller_inputs import TensorClass
...
class AgentWeightedObjectives(AgentObjectives):
    def __init__(self,
                 action: Optional[torch.Tensor] = None,
                 action_weight: Optional[torch.Tensor] = None,
                 goal_consumption: Optional[torch.Tensor] = None,
                 goal_consumption_weight: Optional[torch.Tensor] = None,
                 successor_representation: Optional[torch.Tensor] = None,
                 successor_representation_weight: Optional[torch.Tensor] = None):
        super().__init__(action, goal_consumption, successor_representation)
        self.action_weight = action_weight
        self.goal_consumption_weight = goal_consumption_weight
        self.successor_representation_weight = successor_representation_weight

    def to(self, device):
        super().to(device)
        if self.action_weight is not None:
            self.action_weight = self.action_weight.to(device)
        if self.goal_consumption_weight is not None:
            self.goal_consumption_weight = self.goal_consumption_weight.to(device)
        if self.successor_representation_weight is not None:
            self.successor_representation_weight = self.successor_representation_weight.to(device)

    def shape(self) -> Dict[str, Union[torch.Size, None]]:
        shape_dict = super().shape()
        shape_dict['action_weight'] = self.action_weight.shape
        shape_dict['goal_consumption_weight'] = self.goal_consumption_weight.shape
        shape_dict['successor_representation_weight'] = self.successor_representation_weight.shape
        return shape_dict


class TOMMASPredictions(AgentObjectives):
    def __init__(self,
                 action: torch.Tensor,
...
class TOMMASTransformerPredictions(AgentObjectives):
    def __init__(self,
                 action: torch.Tensor,
                 goal_consumption: torch.Tensor,
                 successor_representation: torch.Tensor,
...
class IterativeTOMMASPredictions(ActionClassification):
    def __init__(self,
                 action: torch.Tensor,
                 embeddings: Union[torch.Tensor, List[torch.Tensor], Tuple[torch.Tensor], None] = None,
